...
```{python}
blended_pred = ((0.2 * lasso.predict(X_test)) + (0.1 * ridge.predict(X_test)) + (0.2 * elasticnet.predict(X_test)) + (0.1 * xgb_gs.predict(X_test)) + (0.4 * mars_model.predict(X_test)))
...
scale_py <- svm_grid %>% pull(scale_factor) %>% unique()
cost_py <- svm_grid %>% pull(cost) %>% unique()
...
testing(train_split) %>%
  ...
  rmse(SalePrice, .pred) %>% 
  pull(.estimate) %>% 
  {glue("For the SVM model in R, the RMSE is {round(., 4)}")}
```
...
#svr = GridSearchCV(SVR(kernel='poly', gamma=0.1),
#                   param_grid={"C": [1e0, 1e1, 1e2, 1e3],
#                               "gamma": np.logspace(-2, 2, 5)})
   
svr = SVR(kernel='poly', C = 0.105, gamma = 0.1)
svr.fit(X_train, y_train)
svr_pred = svr.predict(X_test)
print("For the SVR model in Python, the RMSE is {}".format(rmse(y_test, svr_pred).round(4)))
```
