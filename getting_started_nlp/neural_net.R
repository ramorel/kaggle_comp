...
num_words <- min(50000, length(word_index) + 1)
...
test_dat <- pad_texts[is.na(train_test$target), ]
...
model %>% 
  layer_embedding(input_dim = num_words, 
                  ...
                  trainable = FALSE) %>%
  layer_lstm(64, dropout=0.2, recurrent_dropout=0.2, return_sequences = TRUE) %>%
  layer_lstm(64, dropout=0.2, recurrent_dropout=0.2) %>% 
  layer_global_max_pooling_1d(16) %>% 
...
tibble(
  id = test[["id"]],
  target = round(predict(model, test_dat))) %>% 
...
model %>% 
  layer_embedding(input_dim = num_words,
                  ...
                  input_length = 50,
                  weights = list(embedding_matrix)) %>%
  layer_conv_1d(kernel_size = 16, filters = 5, activation = "relu") %>% 
